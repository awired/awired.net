<?php
/**
* @version $Id: fb_pathway.php 63 2007-04-17 21:23:17Z danialt $
* Fireboard Component
* @package Fireboard
...
?>
    ...
    <div class = "<?php echo $boardclass ?>forum-pathway">
        ...
        if ($forumReviewed)
        {
         ...
         //get viewing	 
		$query= "SELECT w.userid, u.username , k.showOnline FROM #__fb_whoisonline AS w LEFT JOIN #__users AS u ON u.id=w.userid LEFT JOIN #__fb_users AS k ON k.userid=w.userid  WHERE w.link like '%" . $_SERVER['REQUEST_URI'] . "%'";
		$database->setQuery($query);
        ...
        if ($sfunc == "userprofile")
        {
			...
			if ($totalguest > 0) echo '<small style="font-weight:normal;" >('.$totalguest.') '._WHO_ONLINE_GUESTS.'</small>';
        }
