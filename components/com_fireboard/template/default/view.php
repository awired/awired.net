<?php
/**
* @version $Id: view.php 63 2007-04-17 21:23:17Z danialt $
* Fireboard Component
* @package Fireboard
        ...
        <table border = "0" cellspacing = "0" class = "jr-topnav" cellpadding = "0" width="100%">
            <tr>
                <td class = "jr-topnav-left">
                    <?php
                    //go to bottom
                    echo '<a name="forumtop" /> <a href="'.sefRelToAbs("index.php?".$_SERVER["QUERY_STRING"]).'#forumbottom" >';
                        ...
                        if (count($flat_messages) > 0)
                        {
                            foreach ($flat_messages as $fmessage)
                            {
                                ...
                                if ($fbConfig['allowAvatar'])
                                {
                                    ...
                                    elseif ($fbConfig['avatar_src'] == "cb")
                                    {
                                        ...
                                        if ($avatar != '')
                                        {
                                            ...
                                            $msg_avatar = '<span class="fb_avatar"><img src="' . $imgpath . '" /></span>';
                                        //added or modified by mambojoe
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        if ($avatar != '') {
                                            $msg_avatar = '<span class="fb_avatar"><img border="0" src="' . FB_LIVEUPLOADEDPATH . '/avatars/' . $avatar . '" /></span>';
                                        }
										
                                    	else {$msg_avatar = '<span class="fb_avatar"><img  border="0" src="' . FB_LIVEUPLOADEDPATH . '/avatars/s_nophoto.jpg" /></span>'; }
                                    }
                                }
                                ...
                                if ($fbConfig['pm_component'] == "clexuspm" && $fmessage->userid && $my->id)
                                {
                                    ...
                                    {
                                        if ($mostables->aim)
                                            $msg_aim = "<a href=\"aim:goim?screenname=" . str_replace(" ", "+", $mostables->aim) . "\"><img src=\"" . JB_URLEMOTIONSPATH . "aim.png\" border=0></a>";

                                        if ($mostables->icq)
                                            $msg_icq = "<a href=\"http://www.icq.com/whitepages/wwp.php?uin=" . $mostables->icq . "\"><img src=\"" . JB_URLEMOTIONSPATH . "icq.png\" border=0></a>";

                                        if ($mostables->msn)
                                            $msg_msn = "<a href=\"" . sefRelToAbs('index.php?option=com_mypms&task=showprofile&amp;user=' . $PMSName) . "\"><img src=\"" . JB_URLEMOTIONSPATH . "msn.png\" border=0></a>";
                                ...
                                $fb_message_txt = stripslashes(smile::smileReplace($fb_message_txt, 0, $fbConfig['disemoticons'], $smileyList));
            ...
            <table border = "0" cellspacing = "0" class = "jr-topnav" cellpadding = "0" width="100%">
                <tr>
                    <td class = "jr-topnav-left">
                        <?php
                        //go to top
                        echo '<a name="forumbottom" /><a href="'.sefRelToAbs("index.php?".$_SERVER["QUERY_STRING"]).'#forumtop" >';
