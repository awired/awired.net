...
function feedFrontpage( $showFeed ) {
	...
	$info[ 'title' ] 			= $params->def( 'title', 'awired.net rss feed' );
	$info[ 'description' ] 		= $params->def( 'description', 'awired' );
	$info[ 'image_file' ]		= $params->def( 'image_file', 'awired_rss.png' );
	if ( $info[ 'image_file' ] == -1 ) {
		$info[ 'image' ]		= NULL;
	} else{
		$info[ 'image' ]		= $mosConfig_live_site .'/images/M_images/'. $info[ 'image_file' ];
	}
	$info[ 'image_alt' ] 		= $params->def( 'image_alt', 'awired' );
