...
/**
* Utility small for writing the HTML for content
* @package Joomla
* @subpackage Content
*/
class HTML_content {
	...
	*/
	function show( &$row, &$params, &$access, $page=0 ) {
		...
		//var_dump($row);
		$row->text = substr($row->text, strlen($row->introtext));
        //var_dump($row->text);
		...
		*/?>
        ...
        <div class="resume">
            <div style="float:left;">
    			<div style="float:left; padding-right: 5px; padding-top: 5px"><a><img src=""></a></div>
    			<div class="resume-title"><a href="#"><?php HTML_content::Title( $row, $params, $access ); ?></a>
...
global $database;
	...
	echo $numhits;
			...
			<div style="float:right; text-align:right; ">par <?php echo $row->author; ?> le <?php HTML_content::CreateDate( $row, $params ); ?></div>
			<div style="clear:right; text-align:right; ">
			     <?php $results = $_MAMBOTS->trigger( 'onBeforeDisplayContent', array( &$row, &$params, $page ) );
		               echo trim( implode( "\n", $results ) );
		               //echo ampReplace( $row->text ); ?><span style="display:inline; padding-left: 5px"><?php
		               if ($params->get( 'intro_only' ) ) {
		...
		<tr>
			<td valign="top" colspan="2">
				...
				// displays Item Text
				//var_dump($row->text);
				echo ampReplace($row->fulltext.$row->text );
				// qsqs
				?>
			</td>
		</tr>
	...
	function Title( &$row, &$params, &$access ) {
		if ( $params->get( 'item_title' ) ) {
			if ( $params->get( 'link_titles' ) && $row->link_on != '' ) {
				...
				<!--<td class="contentheading<?php echo $params->get( 'pageclass_sfx' ); ?>" width="100%">-->
					<span><a href="<?php echo $row->link_on;?>" class="contentpagetitle<?php echo $params->get( 'pageclass_sfx' ); ?>">
