<?php
/**
* @version $Id: admin.fireboard.php 63 2007-04-17 21:23:17Z danialt $
* Fireboard Component
* @package Fireboard
...
<?php echo _FB_DBMETHOD; ?>
    ...
    <ul>
        ...
        <input name = "installclean" value = "Apply!" type = "button" onclick = "location.href='index2.php?option=com_fireboard&task=installfb&mode=1';"></li>

        <li><label for = "installfromjb"><b><?php echo _FB_DBUPGRADE; ?></b></label>

        <br/>

        <input name = "installfromjb" value = "Apply!" type = "button" onclick = "document.location.href='index2.php?option=com_fireboard&task=installfb&mode=2';"></li>
    </ul>
    </fieldset>
...
function showAdministration($option)
{
    ...
    include_once ("components/com_fireboard/fireboard_config.php");
...
function showConfig($option)
{
    ...
    $ip_opt[] = mosHTML::makeOption( 'none', 'none' );
...
function saveConfig($option)
{
    ...
    $configfile = "components/com_fireboard/fireboard_config.php";
    @chmod($configfile, 0766);
...
function dousersprune($database, $option)
{
    ...
    if (!$ids == "")
    {
        $database->setQuery("DELETE FROM #__fb_users WHERE userid in ($ids)");

        if (!$database->query())
        {
            echo "<script> alert('" . _FB_ERRORPRUNEUSERS . " " . $database->getErrorMsg() . "'); window.history.go(-1); </script>\n";
            exit();
        }

        mosRedirect("index2.php?option=$option&task=pruneusers", "" . _FB_USERSPRUNEDDELETED . "" . $cids . " " . _FB_PRUNEUSERPROFILES . "");
    }
    else
    {
        $cids = 0;
        mosRedirect("index2.php?option=$option&task=pruneusers", _FB_NOPROFILESFORPRUNNING);
    }
}
...
{
    global $travelConfig, $database, $mosConfig_lang;
...
function smileypath()
{
    ...
    if (file_exists($mainframe->getCfg('absolute_path') . '/administrator/components/com_fireboard/fireboard_config.php')) {
        include ($mainframe->getCfg('absolute_path') . '/administrator/components/com_fireboard/fireboard_config.php');
    }
    else {
        die ("Error finding configuration file (" . $mainframe->getCfg('absolute_path') . "/administrator/components/com_fireboard/fireboard_config.php)");
    }
...
function com_install_fireboard($mode = 1)
{
    ...
    add column `class_sfx` varchar(20) NOT NULL  after `headerdesc`, COMMENT='';");
        ...
        if ($database->query()) {
            print "<img src='images/tick.png' align='absmiddle'>Attachment table successfully upgraded to the latest 1.0.x series structure!<br />";
        }
        $database->setQuery("update #__fb_messages_text set message = replace(message,'/components/com_fireboard/uploaded','/images/fbfiles');");
        if ($database->query()) {
            print "<img src='images/tick.png' align='absmiddle'>Attachments in messages table successfully upgraded to the latest 1.0.x series structure!<br />";
        }
...
<a href="index2.php?option=com_fireboard">Click here to continue...</a>
