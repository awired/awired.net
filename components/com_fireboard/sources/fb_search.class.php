<?php
/**
* @version $Id: fb_search.class.php 63 2007-04-17 21:23:17Z danialt $
* Fireboard Component
* @package Fireboard
...
class jbSearch
{
    ...
    function jbSearch(&$database, $search, $uid, $limitstart = 0, $limit = 3)
    {
        ...
        if (empty($allowed_forums))
        {
            ...
            $this->str_FB_errormsg = 'No forums to search in.';
            return 0;
        }
        /* build query */
...
<?php echo _GEN_DATE; ?>
                ...
                foreach ($results as $result)
                {
                    ...
                    $ressubject = preg_replace("/$searchword/i", '<span  class="searchword" >' . $searchword . '</span>', $ressubject);
                    $resmessage = $result->message;
                    $resmessage = preg_replace("/$searchword/i", '{{' . $searchword . '}}', $resmessage);
