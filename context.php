...
if ($pagetype == "article")
{
	...
	if($INFO['writable'])
	{
		if($INFO['draft'])
		{
			$monobook['content_actions']['edit']['href'] = DOKU_BASE.DOKU_SCRIPT."?id=".$ID."&amp;do=draft&amp;rev=".$_REQUEST['rev'];
...
if($conf['useacl'])
{
	if ($_SERVER['REMOTE_USER'])
	{
		$monobook['personal']['userpage']['wiki']=":user:".$_SERVER['REMOTE_USER'];
	...
	else
	{
		...
		$monobook['personal']['login']['text']="Connexion / Créer un compte";
		
		//$monobook['personal']['login']['text']=$lang['btn_login'];
	}
	
		$monobook['personal']['other']['wiki_page']=":wiki:tpl:websites";
		$monobook['personal']['other']['text']="salut!";
		$monobook['personal']['other']['no-endul']=true;
}
...
//if ($name == "p-personal")
//   include "http://gl.awired.net/top.php";
...
if (isset($_REQUEST['rev']))
    $monobook['defaulttoolbox']['print']['href'] = DOKU_BASE."doku.php?id=".$ID."&amp;rev=".$_REQUEST['rev']."&amp;mbdo=print";
else
...
if (isset($_REQUEST['rev']))
    $monobook['defaulttoolbox']['permanent']['href'] = DOKU_BASE."doku.php?id=".$ID."&amp;rev=".$_REQUEST['rev'];
else
...
if (isset($_REQUEST['rev']))
    $monobook['defaulttoolbox']['cite']['href'] = DOKU_BASE."doku.php?id=".$ID."&amp;rev=".$_REQUEST['rev']."&amp;mbdo=cite";
else
